        ...
        for attribute, length_dictionary in list_entry[2].items():
            ...
            if greatest_strlen == 'null':
                ...
                try:
                    ...
                    print(f'The attribute {attribute} in table {table_number + 1} only contains null-values.')
...
def datatypes_visualizer(list, directory):
    ...
    for table_number, table_entry in enumerate(list):
        ...
        values = value_types["Not Null"] + value_types["Null"]
        total = sum(values)
...
def tables_visualizer(list, directory):
    ...
    total_values = sum(workbook_values)
    plt.bar(workbook_tables, workbook_values, color='blue')
    plt.xlabel('Table')
    plt.ylabel('Number of values')
    plt.title(f'Number of values per table ({total_values} values in total)')
...
if len(sys.argv) > 1:
    ...
    if not os.path.exists(workbook_directory):
        raise FileNotFoundError(f"The directory {workbook_directory} you gave as a parameter unfortunately does not exist.")
else:
    raise TypeError("Please give as a parameter the path to a directory which contains an analysis.json file.")
    
json_files = glob.glob(os.path.join(workbook_directory, '*.json'))
if json_files:
    analysis_file = json_files[0]
else:
    raise FileNotFoundError(f"The given directory {workbook_directory} unfortunately does not contain the required analysis.json file with the results of the analysis.")
