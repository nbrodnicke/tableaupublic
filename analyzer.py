...
subprocess.run(terminal_command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
data_directory = new_directory_path.joinpath("Data")
hyper_files = data_directory.glob("**/*.hyper")

for hyper_file in hyper_files:
    if os.path.exists(os.path.join(new_directory_path, os.path.basename(hyper_file))):
        print('There is a duplicate that needs to be fixed.')
    else:
