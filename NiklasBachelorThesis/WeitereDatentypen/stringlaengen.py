            ...
            for schema in connection.catalog.get_schema_names():
                if str(schema.name) == '"Extract"':
                    ...
                    try:
                        ...
                        with connection.execute_query(f"SELECT COUNT(*) FROM {table_names[table_nmb]} WHERE LENGTH({attribute_name}) < 7") as result:
                            rows = list(result)
                            print([rows[0][0]])
...
null_values("First Name", input_hyper_path, 0)