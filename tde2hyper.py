...
from tableauhyperapi import HyperProcess, Telemetry, Connection, CreateMode, TableDefinition, escape_name, \
    escape_string_literal


def _get_external_table_query(tde_file_path: str, schema: str, table: str):
...
def convert_tde_to_hyper(tde_path: Path):
    ...
    print(f"Converting {tde_path} to {hyper_database}...")
    with HyperProcess(telemetry=Telemetry.DO_NOT_SEND_USAGE_DATA_TO_TABLEAU) as hyper:
        with Connection(endpoint=hyper.endpoint, database=hyper_database,
...
if __name__ == '__main__':
    ...
    argparser.add_argument("input_tde_path", type=Path,
                           help="The input TDE file path that will be converted to a Hyper file.")
